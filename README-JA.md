...
このフォークは Seed-VC のリアルタイム推論 GUI の最適化に焦点を当てており、特に VRChat やゲームなどの負荷の高いバックグラウンドアプリを同時に実行する必要がある Windows ユーザーを対象としています。

在 Gemini の助けを借りて **real-time-gui.py** だけを修正しました。
元の **seed-vc** にあるこのファイルを置き換えるだけで、正常に動作します。

主な機能と修正点：

超低 CPU 使用率： リアルタイム係数（RTF）を約 0.014 まで低減しました。負荷の高い AI VAD を置き換え、データパイプラインを最適化することで CPU 使用率を最小限に抑え、他の高負荷ソフトウェアとの同時実行が可能になりました。

マイクゲイン制御： 「Mic Gain」スライダー（0.3倍〜5.0倍）を統合し、処理前に入力信号をデジタル的に増幅または減衰できるようになりました。

高精度な安定性： SOLA（同期重畳加算）フェーズで Float32 精度を強制することで、GPU 加速を維持しつつ、混合精度によるクラッシュを防止します。

シームレスなオーディオストリーム： バッファのローリングとリサンプリングのロジックを最適化し、音質の劣化や「ロボット声」のような蓄積現象なしに、長時間の安定した動作を実現しました。
